...
def drawFinalSamplesPerBudget(stages, restultPath, outPath):
    ...
    for fileName in fileList:
        ...
        for i in range(len(algorithmList)):
            ...
            dfList[i].plot(figsize=(9, 3), style=lStyle, linewidth = 0.8, alpha = 0.7)
        ...
        for i in range(6):
            ...
            if i in range(3):
                ...
                for v in list2Plot:
                    plt.plot(dfList[0][v], dfList[0].index, color = lStyle[v], linewidth=1.2, alpha=0.7)
        ...
        plt.tight_layout()
        plt.savefig(outPath + fileName.replace(restultPath, "").replace(".pickle", "") + "_all2.pdf", transparent=True)
        plt.close()
